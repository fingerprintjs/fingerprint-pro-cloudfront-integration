...
async function cleanupCloudFrontCachePolicies() {
  for await(const policy of listCloudFrontCachePolicies()) {
    try {
      await cloudFront.deleteCachePolicy({ Id: policy.CachePolicy.Id, IfMatch: policy.CachePolicy.ETag }).promise()
...
async function cleanupCloudFrontOriginPolicies() {
  for await(const policy of listCloudFrontOriginPolicies()) {
    try {
      await cloudFront.deleteOriginRequestPolicy({ Id: policy.OriginRequestPolicy.Id, IfMatch: policy.OriginRequestPolicy.ETag }).promise()
