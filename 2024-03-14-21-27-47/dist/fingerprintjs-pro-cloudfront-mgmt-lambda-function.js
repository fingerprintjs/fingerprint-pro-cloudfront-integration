/**
 * FingerprintJS Pro CloudFront Lambda function v2.0.1-rc.2 - Copyright (c) FingerprintJS, Inc, 2024 (https://fingerprint.com)
...
async function handleUpdate(lambdaClient, cloudFrontClient, settings) {
    ...
    const newVersions = Array.from(listVersionsAfterUpdate.values()).filter((conf) => !listVersionsBeforeUpdate.has(conf.Version));
    ...
    if (newVersion.State !== clientLambda.State.Active) {
