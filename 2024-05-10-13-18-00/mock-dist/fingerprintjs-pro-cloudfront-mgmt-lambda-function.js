...
const CLOUDFRONT_CONFIG_UPDATE_ATTEMPT_DELAY = 15000; // Milliseconds
/**
 * @throws {ApiException}
...
async function updateCloudFrontConfig(cloudFrontClient, cloudFrontDistributionId, lambdaFunctionName, latestFunctionArn) {
    ...
    const fpCbs = distributionConfig.CacheBehaviors?.Items?.filter((cb) => fpCDNOrigins?.some((origin) => origin.Id === cb.TargetOriginId));
    if (fpCbs && fpCbs?.length > 0) {
